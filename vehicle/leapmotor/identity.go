...
// sm4Key is the SM4 block cipher key used for PKCS#12 password derivation (extracted from APK).
var sm4Key = [16]byte{0x42, 0x9c, 0xf4, 0x50, 0xef, 0x91, 0x7a, 0x98, 0x54, 0x33, 0x43, 0x0b, 0xcf, 0xed, 0x62, 0xac}

// p12MemoryEncode applies PKCS7 padding then SM4-ECB encryption block by block.
func p12MemoryEncode(data []byte) []byte {
	c, _ := gmsm4.NewCipher(sm4Key[:]) // constant key — error cannot occur
